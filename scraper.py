...
def _extract_post_text(item):
    actualPosts = item.find_all(attrs={"dir": "auto"})
    text = ""
    if actualPosts:
...
def _extract_html(bs_data):
    ...
    k = bs_data.find_all(class_="qzhwtbm6 knvmm38d")
    more = bs_data.find_all(class_="oajrlxb2 g5ia77u1 qu0x051f esr5mh6w e9989ue4 r7d6kgcz rq0escxv nhd2j8a9 nc684nl6 p7hjln8o kvgmc6g5 cxmmr5t8 oygrvhab hcukyx3x jb3vyjys rz4wbd8a qt6c0cv9 a8nywdso i1ao9s8h esuyzwwr f1sip0of lzcic4wl oo9gr5id gpro0wi8 lrazzd5p")
    k = bs_data.find_all(class_="du4w35lb k4urcfbm l9j0dhe7 sjgh65i0")
    postBigDict = []

    for item in k:
        print("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTT",item)
        postDict = dict()
...
def _scroll(browser, infinite_scroll, lenOfPage):
    ...
    while not match:
        ...
        if lastCount == lenOfPage:
            match = True
# "//div[text()='Invite to group via link']"
# from scraper import *
# page = "https://www.facebook.com/groups/supertool"
# list, driver, bs_data = extract(page,10)
# more = driver.find_elements_by_xpath("//div[text()='See More']")
# posts = driver.find_elements_by_xpath("//blockquote")
# full = p.find_element_by_xpath("./../../..").
...
# >>> for p in posts:
# ...     print("================================")
# ...     x = p.find_element_by_xpath("./../../..")
# ...     if p.text is not "":
# ...             for a in x.text.split(p.text):
# ...                     print("$$$$$$$$$$$$$$$$$")
# ...                     print(a)
# ...
...
def extract(page, numOfPost, infinite_scroll=False, scrape_comment=False):
    ...
    if mybrowser is None:
        mybrowser = browser = WhatsAPIDriver(username="wholesomegarden",profile=None).driver
    else:
        browser = mybrowser
    ...
    return postBigDict, browser, bs_data
